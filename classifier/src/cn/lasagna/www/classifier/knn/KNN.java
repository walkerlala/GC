...
public class KNN implements ClassifierInterface {
    ...
    private double descriptionWeight = Configuration.KNNDescriptionWeight;
    ...
    private CompareRecord tfidfValue(Record record) {
        ...
        if (record.getKeywords() != "")
            keywordMap.put("this-is-tfidf-entry", calculateVector(keywordMap));
        else
            keywordMap = null;
        if (record.getTitle() != "")
            titleMap.put("this-is-tfidf-entry", calculateVector(titleMap));
        else
            titleMap = null;
    ...
    private double calculateVector(HashMap<String, Double> map) {
        ...
        for (int i = 0, length = value.size(); i < length; i++)
            sum = sum + value.get(i) * value.get(i);
    ...
    private CompareRecord buildDataModel(Record record) {
     ...
     private HashMap<String, Double>WordFre(String document){
		HashMap<String, Double> map = new HashMap<>();
		String[] content = document.split(",");
		for(String word : content){
            map.merge(word, 1.0, (a, b) -> a + b);//统计tag数
		}
    	return map;
     }
    ...
    private double compareMap(HashMap<String, Double> map1, HashMap<String, Double> map2) {
        ...
        double cos = 0;
        double map1_length = map1.get("this-is-tfidf-entry");
        double map2_length = map2.get("this-is-tfidf-entry");
        double similarity = 0;
        for (String word : map1.keySet()) {
            if (word == "this-is-tfidf-entry")
                continue;
